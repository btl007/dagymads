...
# 개발 업데이트 로그 (2025-12-08)

이 문서는 2025년 12월 8일 수행된 주요 리팩토링 및 기능 개선 사항을 요약합니다.

## 1. 스케줄링 시스템 개선

### 1.1. 관리자용 스케줄 수동 지정 모달 (`AdminSchedulePickerModal.jsx`)
- **목표:** 관리자가 특정 프로젝트의 촬영일을 '날짜'만 지정하는 것이 아니라, 실제 운영 중인 시간 슬롯(Time Slot) 시스템과 연동하여 정확한 '시간'까지 확정할 수 있도록 개선.
- **변경 사항:**
  - 기존의 단순 날짜 선택 캘린더를 제거하고, `UserScheduleModal`을 기반으로 한 관리자 전용 모달 `AdminSchedulePickerModal`을 도입.
  - **슬롯 조회:** `get_all_slots` RPC를 사용하여 해당 월의 모든 슬롯 상태(예약 가능, 대기, 확정 등)를 조회.
  - **즉시 확정:** 관리자가 시간 슬롯을 선택하면 `confirm_schedule_slot` RPC를 호출하여 즉시 예약을 확정 처리.
  - **UX:** 1단계(날짜 선택) -> 2단계(시간 선택)의 직관적인 흐름 제공.

### 1.2. 촬영 관리 페이지 UI 개편 (`AdminVideo.jsx`)
- **목표:** `AdminSchedule` 페이지와 UI 일관성을 맞추고, 정보 가독성을 향상.
- **변경 사항:**
  - **레이아웃 통일:** `AdminSchedule`과 동일한 `Grid (2:1)` 레이아웃 적용. (좌측 캘린더, 우측 상세 정보)
  - **캘린더 스타일:** `date-fns/locale`의 `ko`를 적용하여 한국어 캘린더로 변경하고, 날짜 호버(Hover) 시 `bg-primary/30` 색상을 적용하여 시인성 개선.
  - **상세 카드 디자인:** 우측 패널의 프로젝트 카드를 리디자인. `YYYY-MM-DD HH:mm` 형식의 시간 배지를 상단에 배치하고, 센터명과 프로젝트명을 명확히 구분하여 표시.
  - **데이터 정확성:** `fetchConfirmedProjects` 쿼리에서 `time_slots` 테이블을 조인하여, 단순 날짜(`shootdate`)가 아닌 확정된 슬롯의 정확한 시간(`slot_time`)을 표시하도록 개선.

### 1.3. 스케줄 관리 페이지 UI 개선 (`AdminSchedule.jsx`)
- **변경 사항:**
  - 중복되던 캘린더 상단 헤더(월 이동 버튼 등)를 제거하고, `CustomCalendar` 자체 UI를 활용하여 깔끔하게 통합.
  - 한국어 로컬(`ko`) 적용.

### 1.4. `CustomCalendar.jsx` 리팩토링
- **변경 사항:** `...props`를 통해 상위 컴포넌트에서 전달하는 모든 속성(`locale`, `mode`, `selected` 등)이 내부 `Calendar` 컴포넌트로 올바르게 전달되도록 수정하여 재사용성 향상.

## 2. 데이터 무결성 및 버그 수정

### 2.1. 프로젝트 상세 모달 (`ProjectInfoModal.jsx`)
- **버그:** 프로젝트 상태가 `schedule_fixed`가 아닌 경우(예: 촬영 완료 등), 확정된 촬영일이 있음에도 '미정'으로 표시되는 문제.
- **해결:** 상태 조건(`project.status === 'schedule_fixed'`)을 제거하고, `project.id`만 있다면 항상 `time_slots` 테이블에서 확정된 슬롯을 조회하도록 수정. `maybeSingle()`을 사용하여 데이터가 없는 경우도 안전하게 처리.
- **기능 추가:** '센터 주소' 등 프로필 정보를 수정하고 저장할 때 반영되지 않는 문제 해결 (아래 2.2 참고).

### 2.2. 프로젝트 관리 페이지 (`AdminProject.jsx`)
- **버그:** 프로젝트 정보 수정 시 `projects` 테이블만 업데이트되고, `user_profiles` 테이블(센터 주소, 담당자명 등)은 업데이트되지 않는 문제.
- **해결:**
  - `handleProjectSave` 함수가 `profileChanges` 인자를 받아 `user_profiles` 테이블도 함께 업데이트하도록 로직 추가.
  - `fetchData` 쿼리에 `address` 컬럼 추가.
  - 사용자 피드백을 위해 `alert` 대신 `sonner`의 `toast`를 사용하여 UX 개선.

## 3. 요약 및 향후 계획

- **핵심 성과:** 관리자 스케줄링 프로세스를 슬롯 기반으로 완전히 통합하여 데이터 무결성을 확보하고, UI/UX를 대폭 개선하여 운영 효율성을 높임.
- **다음 단계:** `AdminOverview` 페이지를 단순 목록이 아닌 KPI 중심의 인사이트 대시보드로 전면 개편 예정.


# 개발 업데이트 로그 (2025-12-08 -2)
...
## 2. 통합 로그 시스템 및 데이터 무결성

### 2.1. 통합 감사 로그 시스템 구축 (`audit_logs`)
- **신규 테이블:** `project_activity_logs`를 대체하는 범용 `audit_logs` 테이블 생성.
- **DB 트리거:** `projects`, `user_profiles`, `time_slots`, `scripts` 테이블의 변경(`INSERT`, `UPDATE`, `DELETE`)을 감지하여 자동으로 로그를 남기는 PostgreSQL 트리거 함수(`log_audit_event`) 구현.
- **노이즈 필터링:** `time_slots` 자동 생성과 같은 대량 `INSERT` 작업은 로그에 남기지 않도록 트리거 로직 최적화.
- **Edge Function 연동:** `reset-clerk-user-password` 함수 성공 시 `audit_logs`에 직접 로그를 남기도록 구현.

### 2.2. 로그 뷰어 UI 개편 (`AdminLog.jsx`)
- **데이터 소스 변경:** `audit_logs` 테이블 기반으로 변경.
- **스마트 뷰어:** JSON 데이터를 "누가, 무엇을, 어떻게" 변경했는지 알 수 있는 문장형(Human-readable) UI로 변환.
- **시각화:** 변경 전/후 값을 화살표와 색상(취소선/초록색)으로 구분하고, 액션 유형별 아이콘 적용.
- **데이터 매핑:** `target_id`만 보여주는 대신, 관련 테이블을 추가 조회하여 실제 프로젝트명, 센터명, 시간 슬롯 시간을 표시.

### 2.3. 프로젝트 상세 모달 및 저장 로직 (`ProjectInfoModal.jsx`, `AdminProject.jsx`)
- **버그 수정:** 프로젝트 상태와 무관하게 항상 확정된 촬영일(`confirmed slot`)을 표시하도록 수정.
- **데이터 저장:** 프로젝트 수정 시 `user_profiles` 테이블(주소, 특이사항 등)도 함께 업데이트되도록 로직 개선.
- **데이터 로딩:** `user_profiles` 조회 시 `info`(특이사항) 컬럼이 누락되어 있던 문제를 해결하여 데이터 로딩 정상화.

## 3. 사용자 관리 및 보안

### 3.1. 사용자 목록 페이지 개편 (`AdminUsers.jsx`)
- **UI 개편:** `ul` 리스트를 `shadcn/ui` **Table** 컴포넌트로 교체하여 가독성 향상.
- **기능 추가:** 센터명, ID, 담당자명으로 사용자를 찾을 수 있는 **검색(Search)** 기능 추가.
- **데이터 확장:** `user_profiles`에서 주소(`address`) 정보를 추가로 표시하고 수정 가능하도록 개선.

### 3.2. 비밀번호 초기화 기능
- **기능:** `UserInfoModal`에 '비밀번호 초기화' 버튼 추가.
- **보안:** `reset-clerk-user-password` Edge Function을 통해 관리자(`is_admin`) 권한을 엄격히 확인(JWT Payload 검증)한 후 Clerk API를 호출하여 비밀번호를 재설정.

## 4. 대본 관리 시스템 고도화 (`AdminScript.jsx`)

- **UI/UX 전면 개편:** 기존 3단 컬럼 뷰를 **2단 Inbox 스타일 (목록 + 상세)**로 변경.
- **그룹화:** 대본 목록을 **센터(User)별로 그룹화(Accordion)**하여 관리 효율성 증대.
- **필터링:** 전체/검토 대기/승인됨 탭을 추가하여 업무 흐름에 따른 필터링 제공.
- **데이터 최적화:** `UserCacheContext`를 활용하여 센터명 로딩 성능 및 일관성 확보.
